    ...
    <style>
        ...
        /* Product Mobile Card Dark Mode Support */
                ...
                pageItems.forEach(function(group, idx) {
                    ...
                    <div class="product-mobile-card card border shadow-sm rounded-4 mb-3 p-3 position-relative bg-white">
                        ...
                        <div class="d-flex align-items-center gap-3 mb-2">
                            <img src="${img_url}" alt="${item.product_name}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 10px; border: 1px solid #e2e8f0; flex-shrink: 0;" />
        ...
        function formatProductCode(productCode) {
            try {
                ...
                if (Array.isArray(parsed) && parsed.length > 0) {
                    return parsed.map(code => `<span class="badge bg-light text-success border border-success me-1" style="font-family: monospace; font-size: 12px; font-weight: 600;">${code}</span>`).join('');
                } else if (parsed) {
                    return `<span class="badge bg-light text-success border border-success me-1" style="font-family: monospace; font-size: 12px; font-weight: 600;">${parsed}</span>`;
                }
                return '<span class="text-muted small">N/A</span>';
            } catch (e) {
                return `<span class="badge bg-light text-success border border-success me-1" style="font-family: monospace; font-size: 12px; font-weight: 600;">${productCode || 'N/A'}</span>`;
            }
        }
    </script>