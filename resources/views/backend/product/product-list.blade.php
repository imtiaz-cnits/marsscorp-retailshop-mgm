        ...
        function renderPaginatedList() {
            ...
            if (pageItems.length === 0) {
                ...
                pageItems.forEach(function(group, idx) {
                    ...
                    let isDoorCategory = (item.category && item.category.category_name && item.category.category_name.toLowerCase().includes('door')) || hasHandedness;

                    if (isDoorCategory && hasHandedness) {
                    ...
                    } else if (item.door_side && isDoorCategory) {
                    ...
                    let variantBadgeName = '';
                    if (isMultiVariant) {
                        if (isDoorCategory) {
                            variantBadgeName = `<div class="mt-1"><span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300 dark:border-slate-800"><i class="fa-solid fa-layer-group me-1"></i>${group.items.length} Handedness Variants</span></div>`;
                        } else {
                            variantBadgeName = `<div class="mt-1"><span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-semibold bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:border-slate-800"><i class="fa-solid fa-layer-group me-1"></i>${group.items.length} Stock Records</span></div>`;
                        }
                    } else if (item.door_side && isDoorCategory) {
                        variantBadgeName = `<span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300 dark:border-slate-800 ms-1">${item.door_side}</span>`;
                    }

                    // Modern Action Icons (Sleek minimalist stroke SVG icons, placed on the right of Stock)
                    let actionHtml = '';
                    if (isMultiVariant) {
                        let toggleBtnTitle = isDoorCategory ? `Show / Hide Handedness Variants (${group.items.length} items)` : `Show / Hide Stock Records (${group.items.length} items)`;
                        actionHtml = `
                    ...
                    if (isMultiVariant) {
                        let subRows = group.items.map((subItem, subIdx) => {
                            let subDoorSide = isDoorCategory ? (subItem.door_side || 'Standard') : (subItem.door_side || (`Entry #${subIdx + 1}`));
                            let subDoorIcon = isDoorCategory ?
                                (subDoorSide.toLowerCase().includes('left') ? '👈' : (subDoorSide.toLowerCase().includes('right') ? '👉' : (subDoorSide.toLowerCase().includes('both') ? '↔️' : '🚪'))) :
                                '📦';
                            let subBadgeClass = isDoorCategory ?
                                (subDoorSide.toLowerCase().includes('left') ? 'bg-sky-50 text-sky-700 border-sky-200 dark:bg-sky-950/40 dark:text-sky-300' : (subDoorSide.toLowerCase().includes('right') ? 'bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-950/40 dark:text-indigo-300' : 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300')) :
                        ...
                        let accordionTitle = isDoorCategory ?
                            `<span><strong>${item.product_name}</strong> - Handedness & Stock Variant Details (${group.items.length} Handedness Items)</span>` :
                            `<span><strong>${item.product_name}</strong> - Stock Records & Price Batches (${group.items.length} Records)</span>`;
                        let accordionColHeader = isDoorCategory ? 'Door Side / Handedness' : 'Record / Batch';
                                    ...
                                    <div class="overflow-x-auto rounded-lg border border-slate-200/70 dark:border-slate-800">
                                        <table class="w-full text-left border-collapse text-xs">
                                            <thead class="bg-slate-50 dark:bg-slate-800/80 text-slate-500 dark:text-slate-400 font-semibold border-b border-slate-200/70 dark:border-slate-800">
                                                <tr>
                                                    <th class="px-3 py-2 text-start w-[150px]">${accordionColHeader}</th>
                    ...
                    if (isMultiVariant) {
                        let mobileToggleLabel = isDoorCategory ? `Variant Stock List (${group.items.length} items)` : `Stock Records (${group.items.length} items)`;
                        mobileVariantsHtml = `
                        <div class="border-t border-slate-100 dark:border-slate-800 pt-2.5 mt-2.5">
                            <button type="button" class="btn-outline-success w-full font-semibold flex items-center justify-between px-3 py-2 rounded-xl text-xs bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-slate-800 toggle-variant-btn" data-target="mobile-variant-card-${realIndex}">
                                <span><i class="fa-solid fa-layer-group me-1.5"></i> ${mobileToggleLabel}</span>
