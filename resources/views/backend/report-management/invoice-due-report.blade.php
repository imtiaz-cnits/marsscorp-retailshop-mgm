...
@section('title', 'Invoice Due Report')
@section('content')
    ...
    <script>
        ...
        async function fetchInvoiceReport() {
            ...
            // Fetch the invoice report
            await getList(startDate, endDate);
        }

        async function getList(startDate = '', endDate = '') {
            try {
                showLoader(); // Show loading spinner while fetching data

                // Make the API request with date filters
                ...
                hideLoader(); // Hide loading spinner after fetching data

                let tableList = $("#printTable tbody");
                tableList.empty(); // Clear previous table rows

                // Ensure the data is an array and filter for due orders
                if (Array.isArray(res.data['InvoicePaymentDetails'])) {
                    const dueOrders = res.data['InvoicePaymentDetails'].filter(item => parseFloat(item.due_amount) >
                    0); // Only include orders with due

                    // Loop through filtered due orders
                    dueOrders.forEach(function(item, index) {
                        const subTotal = item['sub_total'] ? parseFloat(item['sub_total']).toFixed(2) : '0.00';
                        const discountAmount = item['discount_amount'] ? parseFloat(item['discount_amount'])
                            .toFixed(2) : '0.00';
                        const paidAmount = item['paid_amount'] ? parseFloat(item['paid_amount']).toFixed(2) :
                            '0.00';
                        const dueAmount = item['due_amount'] ? parseFloat(item['due_amount']).toFixed(2) :
                            '0.00';
                        ...
                        const row = `
                <tr data-row="${index + 1}" data-date="${new Date(item['created_at']).toLocaleDateString()}">
                    <td>${index + 1}</td>
                    <td>${item['order_no']}</td>
                    <td>
                        <div class="client-item">
                            <div class="details">
                                <h1>${item['customer']?.customer_id ?? 'N/A'}</h1>
                                <h1>${item['customer']?.customer_name ?? 'N/A'}</h1>
                                <h2>${item['customer']?.mobile ?? 'N/A'}</h2>
                            </div>
                        </div>
                    </td>
                    <td>
                        <h4>Total: ৳${subTotal}</h4>
                        <h4>Discount: ৳${discountAmount}</h4>
                        <h4>Paid: ৳${paidAmount}</h4>
                        <h4>Due: ৳${dueAmount}</h4>
                    </td>
                    <td>${new Intl.DateTimeFormat('en-US', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(item['created_at']))}</td>
                    <td><span class="badge badge-${paymentStatus === 'Fully Paid' ? 'success' : paymentStatus === 'Partial Paid' ? 'warning' : 'danger'}">${paymentStatus}</span></td>
                    <td>
                        <div id="action_btn">
                            <div id="menu-wrap">
                                ...
                                <div class="menu">
                                    <div>
                                        <ul>
                                            <li>
                                                <button class="btn btn-info" onclick="viewInvoice(${item.id})"><i class="fa-solid fa-print" style="color: #000000;"></i></button>
                                            </li>
                                            <li>
                                                <a href="#" data-id="${item['id']}"
                                                   class="link custom-open-modal-btn editButton"
                                                   data-modal="action-editmodal"><i class="fa-solid fa-pen-to-square"></i></a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                ...
                `;
                    ...
                    console.error("Error: InvoicePaymentDetails is not an array", res.data);
                }

                // Re-initialize event listeners after appending the new rows
                $('.custom-open-modal-btn').on('click', async function() {
                    let id = $(this).data('id');
                    await FillUpUpdateForm(id); // Load data into edit form
                    $("#action-editmodal").modal('show'); // Open edit modal
                });
