                ...
                <div class="card-body product-card-body p-4 sm:p-6 md:p-10">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-3">
                        ...
                        <div class="flex items-center gap-2.5">
                            ...
                            <h1 class="text-xl sm:text-2xl font-bold text-slate-800 dark:text-white tracking-tight leading-none m-0 p-0">Brand List</h1>
                        </div>

                        <!-- Right Controls: + Create Brand Button & Action Buttons with Unified Border -->
...
<style>
    ...
    /* + Create Brand Button Fix: strict 38px height & keep white text on hover */
    #openModalBtns {
    ...
    body[light-mode="dark"] .brand-mobile-card,
    body[light-mode="dark"] .brand-mobile-card img,
    ...
    async function getList() {
        try {
            ...
            if (res.data && res.data.status === "success" && res.data.BrandData) {
                ...
                // Always sort newest first (ID descending) so newly created brands appear on top
    ...
    function renderPaginatedList() {
        ...
        if (pageItems.length === 0) {
            ...
            pageItems.forEach(function(item, idx) {
                    ...
                    <tr data-row="${realIndex + 1}" class="transition-colors">
                        ...
                        <td class="text-center" style="width: 65px !important;">
                            <div class="inline-flex items-center justify-center rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-xs overflow-hidden mx-auto" style="width: 38px !important; height: 38px !important; min-width: 38px !important; min-height: 38px !important; border-radius: 50% !important;">
                                <img alt="${item.name}" src="${logo}" style="width: 38px !important; height: 38px !important; border-radius: 50% !important; object-fit: cover !important; display: block;" onerror="this.src='{{ asset('backend/assets/img/brand-defult-img.svg') }}'">
                            </div>
                        </td>
                        <td class="text-start">
                            <span class="font-bold text-slate-800 dark:text-slate-100 text-sm sm:text-base">${item.name}</span>
                        </td>
                ...
                // Mobile Card View
                let mobileCard = `
                    <div class="brand-mobile-card unified-ui-border rounded-2xl p-3.5 bg-white dark:bg-slate-800/80 shadow-sm mb-3 transition-all">
                        ...
                        <div class="flex items-center gap-3 mb-2.5">
                            <div class="rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex-shrink-0 overflow-hidden flex items-center justify-center shadow-xs" style="width: 40px !important; height: 40px !important; border-radius: 50% !important;">
                                <img src="${logo}" style="width: 40px !important; height: 40px !important; border-radius: 50% !important; object-fit: cover !important;" onerror="this.src='{{ asset('backend/assets/img/brand-defult-img.svg') }}'">
                            </div>
                            <div>
                                <h6 class="font-bold text-slate-800 dark:text-slate-100 text-sm sm:text-base leading-snug">${item.name}</h6>
                                <span class="text-[11px] font-medium text-slate-400 dark:text-slate-500">Brand Item</span>
                            </div>
                        </div>
                        <div class="flex items-center justify-between pt-2.5 mt-1 border-t border-slate-100 dark:border-slate-800">
                            ...
                            <div class="flex items-center gap-2">
                                <button type="button" data-id="${item.id}" class="edit-link w-[32px] h-[32px] rounded-xl bg-emerald-50 hover:bg-emerald-600 text-emerald-600 hover:text-white border border-emerald-200 dark:bg-emerald-950/40 dark:border-slate-800 dark:text-emerald-400 flex items-center justify-center transition-all shadow-sm" data-bs-toggle="modal" data-bs-target="#brandUpdateModal" title="Edit Brand">
                                    <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                                        <path d="M12 20h9"></path>
                                        <path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path>
                                    </svg>
                                </button>
                                <button type="button" data-id="${item.id}" class="custom-delete-modal-btn w-[32px] h-[32px] rounded-xl bg-rose-50 hover:bg-rose-600 text-rose-600 hover:text-white border border-rose-200 dark:bg-rose-950/40 dark:border-slate-800 dark:text-rose-400 flex items-center justify-center transition-all shadow-sm" data-bs-toggle="modal" data-bs-target="#brandDeleteModal" title="Delete Brand">
                                    <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                                        <polyline points="3 6 5 6 21 6"></polyline>
                                        <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                                    </svg>
