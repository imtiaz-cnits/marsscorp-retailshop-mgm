...
@section('title', 'Supplier Due Payable Page')
@section('content')
                            ...
                            <table id="printTable" class="table table-bordered table-hover">
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td id="totalPayableaamountPrice">0</td>
                                        <td></td>
                                        <td></td>
                                    </tr>
                                </tfoot>
    ...
    <script>
        async function getList(startDate = '', endDate = '') {
            try {
                ...
                let url = `/api/supplier-list-report-due-amount?startDate=${startDate}&endDate=${endDate}`;
                let res = await axios.get(url, HeaderToken()); // Fetch supplier list
                hideLoader(); // Hide loader after response

                let tableList = $("#tableList");
                let totalPayableAmount = 0; // Initialize total amount
                tableList.empty(); // Clear previous table content

                res.data['SupplierData'].forEach(function(item, index) {
                    if (!item['purchase_payable_amount'] || item['purchase_payable_amount'] <= 0) {
                        return; // Skip suppliers with no payable amount
                    }

                    const img_url = item['img_url'] ? item['img_url'] :
                        "{{ asset('back-end/assets/img/demo-img.jpeg') }}";
                    let payableAmount = parseFloat(item['purchase_payable_amount']) || 0;
                    totalPayableAmount += payableAmount; // Add to total

                    let row = `
                <tr data-row="${index + 1}">
                    <td>${index + 1}</td>
                    <td>
                        <div id="action_btn_wrap">
                            <a href="#" data-id="${item['id']}" class="link edit-link"
                               data-bs-toggle="modal" data-bs-target="#exampleModal">
                               <i class="fa-solid fa-pen-to-square"></i>
                            </a>
                            <a href="#" data-id="${item['id']}" class="link custom-delete-modal-btn"
                    ...
                    <td><img style="width: 60px; height: auto" alt="${item['name']}" src="${img_url}"></td>
                    <td>${item['name']}</td>
                    <td>${item['company']}</td>
                    <td>৳ ${payableAmount.toFixed(2)}</td> <!-- Formatted amount -->
                    <td>${item['status']}</td>
                </tr>
            `;
                ...
                // ✅ Update Total Purchase Payable Amount
        ...
        // Function to fetch filtered data on button click
        ...
        // ✅ Fetch initial list on page load
        getList();
    </script>
