    ...
    <script>
        getList();
        // Function to fetch and display the locationlist
        async function getList() {
            try {
                showLoader(); // Show loader during API request
                let res = await axios.get("/api/location-list", HeaderToken()); // Fetch locationlist
                hideLoader(); // Hide loader after response

                let tableList = $("#tableList");
                tableList.empty(); // Clear previous table content

                res.data['LocationData'].forEach(function(item, index) {
                    let row = `
                        <tr data-row="${index + 1}">
                            <td>${index + 1}</td>
