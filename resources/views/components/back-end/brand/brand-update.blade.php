                ...
                <form id="updateBrandForm" onsubmit="BrandUpdateSave(event)">
                    ...
                    <div class="card border-0 shadow-sm rounded-3 p-3 mb-3 text-center bg-white">
                        <div class="d-inline-block position-relative mb-2">
                            <img id="updateBrandShowImage" src="{{ asset('back-end/assets/img/brand-defult-img.svg') }}" alt="Brand Logo Preview" style="width: 85px; height: 85px; object-fit: contain; border-radius: 12px; border: 2px dashed #86efac; padding: 4px; background: #f0fdf4;" />
                        </div>
                        <div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        $('#brandUpdateModal').on('hidden.bs.modal', function () {
            ...
            $('#updateBrandID').val('');
            $('#updateBrandShowImage').attr('src', "{{ asset('back-end/assets/img/brand-defult-img.svg') }}");
        });
    });
    ...
    async function FillUpUpdateForm(id) {
        try {
            ...
            if (res.data && res.data.status === "success" && res.data.rows) {
                ...
                const logo = data.logo ? (data.logo.startsWith('http') ? data.logo : '/' + data.logo.replace(/^\/+/, '')) : "{{ asset('back-end/assets/img/brand-defult-img.svg') }}";
