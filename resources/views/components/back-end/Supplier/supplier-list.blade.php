                    ...
                    <div class="card-body">
                        <div class="button-wrapper mb-3">
                            ...
                            <div class="button-item">
                                <button id="openModalBtns" onclick="openSupplierCreateModal()" type="button" class="create-invoice">
                                    + Create Supplier
                                </button>
                            ...
                            <table id="printTable" class="table table-bordered table-hover align-middle">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th class="text-start text-nowrap" style="min-width: 130px;">Supplier ID</th>
                                ...
                                <tfoot class="table-light fw-bold">
                                    <tr>
                                        ...
                                        <td id="totalPayableaamountPrice" class="text-end fw-bold text-danger">৳ 0.00</td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            </table>
...
<style>
    /* Supplier Mobile Card & Dark Mode Styling */
    body[light-mode="dark"] .supplier-mobile-card {
        background-color: #1e293b !important;
        border-color: #334155 !important;
        color: #f8fafc !important;
    }

    body[light-mode="dark"] .supplier-mobile-card .text-dark,
    body[light-mode="dark"] .supplier-mobile-card h6 {
        color: #f8fafc !important;
    }

    body[light-mode="dark"] .supplier-mobile-card .bg-light {
        background-color: #0f172a !important;
        border-color: #334155 !important;
    }

    body[light-mode="dark"] .supplier-mobile-card .text-muted {
...
<script>
        ...
        function renderPaginatedList() {
            ...
            $("#totalPayableaamountPrice").text(`৳ ${totalPayableAmount.toFixed(2)}`);
            ...
            if (pageItems.length === 0) {
                tableList.html('<tr><td colspan="8" class="text-center text-danger p-4 fw-bold">❌ কোনো সাপ্লায়ার পাওয়া যায়নি।</td></tr>');
                mobileCardList.html('<div class="p-4 text-center text-danger fw-bold bg-white rounded-3 border shadow-sm">❌ কোনো সাপ্লায়ার পাওয়া যায়নি।</div>');
            } else {
                pageItems.forEach(function (item, idx) {
                    ...
                    let row = `
                        <tr data-row="${realIndex + 1}">
                            ...
                            <td class="text-nowrap">
                                <a href="/supplier/profile/${item['id']}" class="fw-bold text-success text-decoration-none text-nowrap" style="white-space: nowrap;">
                                    <i class="fa-solid fa-truck-field me-1"></i>${item['supplier_id']}
                                </a>
                            </td>
                            <td class="text-center">
                                <img style="width: 45px; height: 45px; object-fit: cover;" class="rounded-circle border shadow-sm" alt="${item['name']}" src="${img_url}" onerror="this.src='{{ asset('back-end/assets/img/demo-img.jpeg') }}'">
                            </td>
                            <td>
                    ...
                    // Mobile Card View
                    let mobileCard = `
                        <div class="supplier-mobile-card card border shadow-sm rounded-4 mb-3 p-3 position-relative bg-white">
                            <div class="d-flex align-items-center justify-content-between pb-2 mb-2 border-bottom">
                                <div class="d-flex align-items-center gap-1">
                                    ...
                                    <span class="badge bg-light text-dark border fw-bold" style="font-size: 11px;">
                                        <i class="fa-solid fa-truck-field me-1 text-success"></i>${item['supplier_id']}
                                    </span>
                                </div>
                                <div>
                            ...
                            <div class="d-flex align-items-center gap-3 mb-2">
                                <img src="${img_url}" class="rounded-circle border shadow-sm" style="width: 48px; height: 48px; object-fit: cover;" onerror="this.src='{{ asset('back-end/assets/img/demo-img.jpeg') }}'">
                                <div>
                                    <h6 class="fw-bold text-dark mb-0" style="font-size: 14px;">${item['name']}</h6>
                                    <span class="text-muted small" style="font-size: 11px;">
                                        <i class="fa-solid fa-building me-1"></i>${item['company'] || 'No Company'}
                                    </span>
                                </div>
                            </div>

                            <div class="bg-light p-2 rounded-3 my-2 d-flex align-items-center justify-content-between" style="border: 1px solid #f1f5f9;">
                                <span class="text-muted small fw-bold" style="font-size: 11px;">বকেয়া দেয় (Payable):</span>
                                <span class="fw-bold fs-6 ${payableAmount > 0 ? 'text-danger' : 'text-dark'}">৳ ${payableAmount.toFixed(2)}</span>
                            </div>

                            <div class="d-flex align-items-center justify-content-between pt-2 mt-1 border-top">
                                <a href="/supplier/profile/${item['id']}" class="btn btn-sm btn-outline-primary rounded-pill px-3 fw-bold" style="font-size: 11px;">
                                    <i class="fa-solid fa-user-circle me-1"></i>প্রোফাইল
                                </a>
                                <div class="d-flex align-items-center gap-2">
                                    <a data-id="${item['id']}" href="#" class="edit-link btn btn-sm btn-outline-success rounded-circle d-flex align-items-center justify-content-center p-0" data-bs-toggle="modal" data-bs-target="#exampleModal" title="Edit" style="width: 34px; height: 34px;">
                                        <i class="fa-solid fa-pen-to-square fs-6"></i>
                                    </a>
                                    <a href="#" data-id="${item['id']}" class="custom-delete-modal-btn btn btn-sm btn-outline-danger rounded-circle d-flex align-items-center justify-content-center p-0" data-bs-toggle="modal" data-bs-target="#confirmationModal" title="Delete" style="width: 34px; height: 34px;">
            ...
            // Delete button listener (matches product-list.blade.php)
