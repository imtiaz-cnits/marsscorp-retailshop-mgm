                    ...
                    <div class="table-responsive d-none d-md-block">
                        <table id="printTable" class="table table-bordered table-hover align-middle">
                            <thead class="table-light">
                                <tr>
                                    <th class="text-center" style="width: 70px;">Serial No</th>
...
<script>
    ...
    function renderPaginatedList() {
        ...
        if (pageItems.length === 0) {
            tableList.html('<tr><td colspan="5" class="text-center text-danger p-4 fw-bold">❌ কোনো ক্যাটাগরি পাওয়া যায়নি।</td></tr>');
            mobileCardList.html('<div class="p-4 text-center text-danger fw-bold bg-white rounded-3 border shadow-sm">❌ কোনো ক্যাটাগরি পাওয়া যায়নি।</div>');
        } else {
            pageItems.forEach(function(item, idx) {
                let realIndex = startIndex + idx;
                let img = item.img_url ? (item.img_url.startsWith('http') ? item.img_url : '/' + item.img_url.replace(/^\/+/, '')) : "{ asset('back-end/assets/img/category-defult-img.svg') }";
                let statusBadgeClass = (item.status === 'Active' || item.status === 'active')
                    ...
                    <tr data-row="${realIndex + 1}">
                        ...
                        <td class="text-center">
                            <div class="d-flex align-items-center justify-content-center gap-1">
                                <button type="button" data-id="${item.id}" class="edit-link btn btn-sm btn-outline-success rounded-circle d-flex align-items-center justify-content-center p-0" data-bs-toggle="modal" data-bs-target="#categoryUpdateModal" title="Edit Category" style="width: 32px; height: 32px;">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </button>
                                <button type="button" data-id="${item.id}" class="custom-delete-modal-btn btn btn-sm btn-outline-danger rounded-circle d-flex align-items-center justify-content-center p-0" data-bs-toggle="modal" data-bs-target="#categoryDeleteModal" title="Delete Category" style="width: 32px; height: 32px;">
                                    <i class="fa-solid fa-trash"></i>
                                </button>
                            </div>
                        </td>
                        <td class="text-center">
                            <img style="width: 45px; height: 45px; object-fit: contain;" class="rounded-3 border shadow-sm p-1 bg-white" alt="${item.category_name}" src="${img}" onerror="this.src='{ asset('back-end/assets/img/category-defult-img.svg') }'">
                        </td>
                        <td class="text-start">
                            <span class="fw-bold text-dark fs-6">${item.category_name}</span>
                        </td>
                        <td class="text-center">
                            <span class="badge ${statusBadgeClass} px-3 py-1 fw-bold" style="font-size: 11px; border-radius: 12px;">
                                ${item.status || 'Active'}
                            </span>
                        </td>
                    </tr>`;
                ...
                // Mobile Card View
                let mobileCard = `
                    <div class="category-mobile-card card border shadow-sm rounded-4 mb-3 p-3 position-relative bg-white">
                        ...
                        <div class="d-flex align-items-center gap-3 mb-2">
                            <img src="${img}" class="rounded-3 border shadow-sm p-1 bg-white" style="width: 48px; height: 48px; object-fit: contain;" onerror="this.src='{ asset('back-end/assets/img/category-defult-img.svg') }'">
                            <div>
                                <h6 class="fw-bold text-dark mb-0" style="font-size: 15px;">${item.category_name}</h6>
                            </div>
                        </div>
                        <div class="d-flex align-items-center justify-content-end gap-2 pt-2 mt-1 border-top">
                            <button type="button" data-id="${item.id}" class="edit-link btn btn-sm btn-outline-success rounded-circle d-flex align-items-center justify-content-center p-0" data-bs-toggle="modal" data-bs-target="#categoryUpdateModal" title="Edit Category" style="width: 34px; height: 34px;">
                                <i class="fa-solid fa-pen-to-square fs-6"></i>
                            </button>
                            <button type="button" data-id="${item.id}" class="custom-delete-modal-btn btn btn-sm btn-outline-danger rounded-circle d-flex align-items-center justify-content-center p-0" data-bs-toggle="modal" data-bs-target="#categoryDeleteModal" title="Delete Category" style="width: 34px; height: 34px;">
